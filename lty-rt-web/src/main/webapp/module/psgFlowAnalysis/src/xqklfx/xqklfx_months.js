define(
		[ 'text!psgFlowAnalysis/tpl/xqklfx/xqklfx_months.html' ],
		function(tpl) {
			var 	chartData;
			var self = {
				dateRange1 : [],
				...
				show : function() {
					...
					$('#btn_hours_confirm').click(
							function() {
								...
								comm.request('/report/areaAnalysis/xlfkfxForMonths',params,function(resp){
				...
				exportData : function(){
					...
					if(psgType == 'option1'){
						titleColumn = ["AREA_NAME","LINE_NO","P_NAME","MM","ONBUS_PERSON_COUNT","OFFBUS_PERSON_COUNT"];
						titleName = ["小区","线路","站台","时间","上车人数","下车人数"];
					}else if(psgType == 'option2'){
						titleColumn = ["AREA_NAME","TAGET_AREA_NAME","MM","ONBUS_PERSON_COUNT"];
						titleName = ["源小区","目标小区","时间","小区间客流量"];
					}
					
					$("#img").val(img);
				    $("#tableData").val(JSON.stringify(self.data));
				    $("#fileName").val("区域客流统计分析-按月");
