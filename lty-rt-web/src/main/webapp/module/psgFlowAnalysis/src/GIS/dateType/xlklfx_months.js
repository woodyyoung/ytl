define(['text!psgFlowAnalysis/tpl/GIS/dateType/xlklfx_months.html'],
function(tpl) {
    var self = {
        mapObj: null,
        infoWindow: null,
        ...
        show: function(data) {
            ...
            //确定按钮
            $('#btn_month_confirm').click(function() {
                self.initData();
            });
        ...
        initData: function() {
            ...
            var startTime =  moment($('#startTime').val()).startOf('month').format('YYYY-MM-DD');
            var endTime =  moment($('#endTime').val()).endOf('month').format('YYYY-MM-DD');
            params = {
                "dataType": "M",
                "lineId": lineId,
                "holidayFlag": dayType,
        ...
        initLineEChats: function(data) {
            ...
            if (json != null && json != undefined) {
                for (var i = 0; i < json.length; i++) {
                    ...
                    if (!comm.contains(xAxisData, item.MM)) {
                        xAxisData.push(item.MM);
                    }
                    if (!comm.contains(legendData, item.LINE_NAME)) {
                        legendData.push(item.LINE_NAME);
                    }
                }
            }
