define([ 'text!psgFlowAnalysis/tpl/klfxtd/ztklfx_weeks.html' ],
		function(tpl) {
			var chartData;
			var self = {
				dateRange1 : [],
				...
				show : function() {
					...
					$('#btn_hours_confirm').click(
							function() {
								...
								comm.request('/report/template/ztfkfxForWeeks',params, function(resp) {
									if(resp.data){
				...
				getDateRange : function() {
					self.dateRange1 = [];
					self.dateRange2 = [];
					var begin_date1 = moment($('#begin_date').val()).isoWeekday(1);
					var end_date1 = moment($('#end_date').val()).isoWeekday(7);
					var begin_date2 = moment($('#begin_date2').val()).isoWeekday(1);
				...
				exportData : function(){
					...
					if(psgType == '0'){
						titleColumn = ["P_NAME","IW","ONBUS_PERSON_COUNT","OFFBUS_PERSON_COUNT"];
						titleName = ["站台","时间","上车人数","下车人数"];
					}else if(psgType == '1'){
						titleColumn = ["SOURCE_PLATFORM_NAME","TAGET_PLATFORM_NAME","IW","ONBUS_PERSON_COUNT"];
						titleName = ["源站台","目标站台","时间","站台间客流量"];
					}
					
					$("#img").val(img);
				    $("#tableData").val(JSON.stringify(self.data));
				    $("#fileName").val("站台客流统计分析-按周");
