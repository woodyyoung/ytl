define([ 'text!psgFlowAnalysis/tpl/ztklcx/ztklcx_weeks.html' ],
		function(tpl) {
	        var 	chartData;
			var self = {
				dateRange1 : [],
				...
				show : function() {
					...
					$('#btn_hours_confirm').click(function() {
							...
							comm.request('/report/template/ztfkfxForWeeks',
				...
				initSelect : function() {
					...
					var titleColumn = ["P_NAME","IW","ONBUS_PERSON_COUNT","OFFBUS_PERSON_COUNT"];
...
//						if (!comm.contains(self.xAxisData, item.IW)) {
//							self.xAxisData.push(item.IW);
//						}
//					}
//
//
//					var	title = $("#yztselect").select2("data")[0].text;
//					
//
//					self.myChart.setOption({
//						title : {
...
//							  xAxisData.push("第"+(i+1)+"周");//横坐标赋值
							xAxisData.push(dataTotile[i].IW+"周");
				         	}
						//三种值得存在 series设置
					      if(begin_date&&end_date){
