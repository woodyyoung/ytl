define(
		[ 'text!psgFlowAnalysis/tpl/xlklfx/xlklfx_days.html' ],
		function(tpl) {
			var 	chartData;
			var self = {
				dateRange1:[],
					...
					$('#xlklfx_export').click(function(){
					 ...
					 $('#btn_hours_confirm').click(function(){
						...
						if (!flag) {
						 ...
						 comm.request('/report/lineAnalysis/xlfkfxFordays',params,function(resp){
				...
				exportData : function(){
					...
					if(psgType == 'option1'){
						titleColumn = ["LINE_NAME","STATION_NAME","DD","ONBUS_PERSON_COUNT","OFFBUS_PERSON_COUNT"];
						titleName = ["线路","站点","时间","上车人数","下车人数"];
					}else if(psgType == 'option2'){
						titleColumn = ["LINE_NAME","STATION_NAME","TAGET_STATION_NAME","DD","ONBUS_PERSON_COUNT"];
						titleName = ["线路","源站台","目标站台","时间","站台间客流量"];
					}
					
					$("#img").val(img);
				    $("#tableData").val(JSON.stringify(self.data));
				    $("#fileName").val("线路客流统计分析-按日");
