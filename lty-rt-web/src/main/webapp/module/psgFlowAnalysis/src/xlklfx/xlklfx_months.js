define(
		[ 'text!psgFlowAnalysis/tpl/xlklfx/xlklfx_months.html' ],
		function(tpl) {
			var 	chartData;
			var self = {
				dateRange1 : [],
				...
				show : function() {
					...
					$('#btn_hours_confirm').click(
							function() {
								...
								comm.request(
										'/report/lineAnalysis/xlfkfxForMonths',
				...
				exportData : function(){
					...
					if(psgType == 'option1'){
						titleColumn = ["LINE_NAME","STATION_NAME","MM","ONBUS_PERSON_COUNT","OFFBUS_PERSON_COUNT"];
						titleName = ["线路","站点","时间","上车人数","下车人数"];
					}else if(psgType == 'option2'){
						titleColumn = ["LINE_NAME","STATION_NAME","TAGET_STATION_NAME","MM","ONBUS_PERSON_COUNT"];
						titleName = ["线路","源站台","目标站台","时间","站台间客流量"];
					}
					
					$("#img").val(img);
				    $("#tableData").val(JSON.stringify(self.data));
				    $("#fileName").val("线路客流统计分析-按月");
